...
export async function createExpense(expenseData, userId, memberIds) {
  try {
    ...
    console.error('Error creating expense in Appwrite:', error);
    throw new Error(error.message);
  }
}
...
export async function getAllExpenses(userId) {
  try {
    const response = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.expensesCollectionId,
      [Query.equal("payer", userId)]
    );
    return response.documents;
  } catch (error) {
    console.error('Error retrieving expenses from Appwrite:', error);
    throw new Error(error.message);
  }
}
